...
class AsyncIndexTestCase(unittest.TestCase):
    def test_run_add_documents_to_schema(self):
        ...
        async_index.run_add_documents_to_schema('index1', 'schema1', bodies)
        sleep(5)
    ...
    def test_run_get_documents_by_id(self):
        ...
        ids = []
        for body in bodies:
            ids.append(index.add_document('index1', 'schema1', body).json()['_id'])
        sleep(2)
        self.assertEqual(5, len(async_index.run_get_documents_by_id('index1', 'schema1', ids)))